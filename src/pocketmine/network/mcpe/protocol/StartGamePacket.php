...
class StartGamePacket extends DataPacket
{
	...
	public $telemetryServerId = "";
	public $telemetryScenarioId = "";
	public $telemetryWorldId = "";
	public $telemetryOwnerId = "";
							...
							if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_370) {
								...
								if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_407) {
									...
									if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_544) {
										...
										if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_685 && $this->getProtocol() < ProtocolInfo::PROTOCOL_924) {
												...
												if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_589) {
													$this->networkPermissions = NetworkPermissions::decode($this);
															if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_924) {
																$hasServerJoinInformation = $this->getBool();
																if ($hasServerJoinInformation) {
																	// The full join info structure doesn't exist in this fork yet.
																	// We don't expect to decode this packet from clients.
																	throw new PacketDecodeException("Server join information decoding is not implemented");
																}
																$this->telemetryServerId = $this->getString();
																$this->telemetryScenarioId = $this->getString();
																$this->telemetryWorldId = $this->getString();
																$this->telemetryOwnerId = $this->getString();
															}
														}
													}
												}
									...
									if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_544) {
										...
										if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_582) {
											...
											if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_589) {
												...
												if ($this->getProtocol() >= ProtocolInfo::PROTOCOL_924) {
													$this->writeOptional(null, fn () => null);
													$this->putString($this->telemetryServerId !== "" ? $this->telemetryServerId : $this->serverIdentifier);
													$this->putString($this->telemetryScenarioId !== "" ? $this->telemetryScenarioId : $this->scenarioIdentifier);
													$this->putString($this->telemetryWorldId !== "" ? $this->telemetryWorldId : $this->worldIdentifier);
													$this->putString($this->telemetryOwnerId !== "" ? $this->telemetryOwnerId : $this->ownerIdentifier);
												}
											}
										}
									}
