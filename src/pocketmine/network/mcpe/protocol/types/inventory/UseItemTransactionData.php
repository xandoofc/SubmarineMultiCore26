...
class UseItemTransactionData extends TransactionData
{
	...
	protected function encodeData(NetworkBinaryStream $stream, int $playerProtocol) : void
	{
		...
		if ($playerProtocol >= ProtocolInfo::PROTOCOL_975) {
			$stream->putVarInt($this->blockPos->x);
			$stream->putVarInt($this->blockPos->y);
			$stream->putVarInt($this->blockPos->z);
		} else {
	 ...
	 */
	public static function new(array $actions, int $actionType, TriggerType $triggerType, Vector3 $blockPos, int $face, int $hotbarSlot, ItemStackWrapper $itemInHand, Vector3 $playerPos, Vector3 $clickPos, int $blockRuntimeId, PredictedResult $clientPrediction, int $clientCooldownState = 0) : self
	{
		$result = new self();
