...
class QueryRegenerateEvent extends ServerEvent
{
	...
	public function __construct(Server $server, int $timeout = 5)
	{
		...
		$this->customServerName = $server->getSubmarineProperty("query.server-name", "Submarine");
		$this->motd = $server->getMotd();
		$this->subMotd = $server->getName() . " v" . $server->getPocketMineVersion();
		$this->listPlugins = $server->getSubmarineProperty("query.visible-plugins", true);
		$this->visiblePlayers = $server->getSubmarineProperty("query.visible-players", false);
		$this->plugins = $server->getPluginManager()->getPlugins();
		$this->players = [];
		$onlinePlayers = $server->getOnlinePlayers();
		if ($server->getSubmarineProperty("query.visible-players", false)) {
		...
		$this->map = $server->getSubmarineProperty("query.visible-map", false) ? $server->getDefaultLevel() === null ? "unknown" : $server->getDefaultLevel()->getName() : "world";
